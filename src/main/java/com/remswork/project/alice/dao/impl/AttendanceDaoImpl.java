...
public class AttendanceDaoImpl implements AttendanceDao {
    ...
    public List<Attendance> getAttendanceListByStudentAndSubjectId(long studentId, long subjectId, long termId)
        ...
        try {
            List<Attendance> attendanceList = new ArrayList<>();
            String hql =
                    "from Attendance where student.id = :studentId and subject.id = :subjectId and term.id = :termId";
    ...
    public Attendance addAttendance(Attendance attendance, long studentId, long subjectId, long termId)
        ...
        try {
            ...
            if (subject == null)
                throw new GradingFactorException("Attendance's subject with id : " + subjectId + " does not exist");
            if (term == null)
                throw new GradingFactorException("Attendance's term with id : " + termId + " does not exist");
            if (attendance.getDate() == null)
                throw new GradingFactorException("Attendance's date is required");
            if (attendance.getDate().trim().equals(""))
                throw new GradingFactorException("Attendance can't have an empty date");
